...
export const whatsappApi = {
  async createMessageTemplate(template: WhatsAppTemplate) {
    try {
      ...
      const url = `${WHATSAPP_API_BASE}/${process.env.WHATSAPP_API_VERSION}/${process.env.WHATSAPP_BUSINESS_ACCOUNT_ID}/message_templates`;
  ...
  async uploadMedia(file: File, type: string) {
    try {
      const uploadUrl = `${WHATSAPP_API_BASE}/${process.env.WHATSAPP_API_VERSION}/${process.env.WHATSAPP_PHONE_NUMBER_ID}/media`;
      ...
      const verifyUrl = `${WHATSAPP_API_BASE}/${process.env.WHATSAPP_API_VERSION}/${mediaId}`;
      let attempts = 0;
